...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Launch Simplenote MCP Server with Python 3.13+ compatibility")
    ...
    for file in [pid_file, alt_pid_file]:
        if file.exists():
            try:
                ...
                logger.warning(f"   To stop it, run: ./simplenote_mcp/scripts/cleanup_servers.sh")
                return True
...
def run_server():
    ...
    try:
        ...
        process = Popen(cmd, stdout=PIPE if not args.verbose else None, 
            ...
            if process and process.poll() is None:
                ...
                try:
                    ...
                    if process.poll() is None:
                        logger.warning("Server not responding to terminate signal, force killing...")
                        process.kill()
