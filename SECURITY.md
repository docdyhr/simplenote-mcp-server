...
- **Health/metrics HTTP endpoint**: `/health`, `/ready`, `/metrics`, and `/thresholds` are loopback-only by default (`HTTP_HOST=127.0.0.1`). The server refuses to start `ENABLE_HTTP_ENDPOINT=true` on a non-loopback `HTTP_HOST` unless `HTTP_ENDPOINT_AUTH_TOKEN` (a bearer token, checked via constant-time comparison, same mechanism as `MCP_HTTP_AUTH_TOKEN`) is configured — mirroring the MCP HTTP transport's own guard. Loopback callers (including the container's own `exec`-based liveness/readiness probe) are always trusted regardless of whether a token is set, so enabling the token never breaks local health checks.
