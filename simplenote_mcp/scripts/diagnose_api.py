...
SIMPLENOTE_API_URL = "https://app.simplenote.com"  # Main site URL for connectivity check
SIMPERIUM_API_HOST = "api.simperium.com"  # Actual API endpoint
DEFAULT_TIMEOUT = 10  # seconds
DIAGNOSTIC_REPORT_PATH = Path(PROJECT_ROOT) / "simplenote_mcp" / "logs" / "api_diagnostic_report.txt"
...
def test_internet_connectivity() -> bool:
    ...
    try:
        ...
        print_and_log("✅ Internet connectivity check passed - successfully connected to Google DNS")
        return True
...
def test_dns_resolution() -> bool:
    ...
    for domain in domains:
        try:
            ...
            print_and_log(f"  This suggests DNS issues or the {domain} domain is unreachable")
            success = False
        ...
        try:
            with socket.create_connection((domain, 443), timeout=DEFAULT_TIMEOUT) as sock, \
                 context.wrap_socket(sock, server_hostname=domain) as ssock:
                    ...
                    print_and_log(f"  Certificate issuer: {issuer.get('organizationName', 'Unknown')}")
                    print_and_log(f"  Certificate subject: {subject.get('commonName', 'Unknown')}")
                    print_and_log(f"  Certificate valid until: {cert.get('notAfter', 'Unknown')}")
        except (socket.error, ssl.SSLError) as e:
            print_and_log(f"❌ TLS connection failed to {domain}: {e}")
            print_and_log(f"  This suggests TLS/SSL issues or the {domain} endpoint is unreachable")
            success = False
...
def test_http_connection() -> bool:
    ...
    try:
        ...
        if e.code in (301, 302, 303, 307, 308):  # Redirects
            print_and_log(f"✅ HTTP connection successful to Simplenote website (received redirect {e.code})")
            print_and_log(f"  Headers: {dict(e.headers) if hasattr(e, 'headers') else 'No headers'}")
            return True
        else:
            print_and_log(f"❌ HTTP connection failed with unexpected status: {e.code} {e.reason}")
            print_and_log(f"  Headers: {dict(e.headers) if hasattr(e, 'headers') else 'No headers'}")
            return False
        ...
        print_and_log(f"❌ HTTP connection failed with unexpected error: {type(e).__name__}: {e}")
        return False
...
def test_api_auth() -> bool:
    ...
    redacted_email = f"{email[:3]}{'*' * (len(email) - 6)}{email[-3:]}" if len(email) > 6 else "***"
    ...
    try:
        ...
        try:
            ...
            if status == 0:
                ...
                print_and_log(f"❌ Authentication failed with status code {status} (took {elapsed:.2f}s)")
    ...
    try:
        ...
        if hasattr(simplenote, 'Simplenote'):
            ...
            try:
                ...
                for method in ['get_note_list', 'get_note', 'update_note', 'add_note', 'trash_note', 'delete_note']:
                ...
                print_and_log(f"❌ Error creating Simplenote instance: {type(e).__name__}: {e}")
                return False
        else:
...
def test_proxy_settings() -> None:
    ...
    if proxies["http"] != "None" or proxies["https"] != "None":
        ...
        print_and_log("  If you're experiencing issues, try temporarily unsetting these variables")
    else:
...
def test_api_performance() -> None:
    ...
    try:
        ...
        if success_count == len(results):
            print_and_log(f"✅ All API performance tests passed ({success_count}/{len(results)})")
        else:
            print_and_log(f"⚠️  Some API performance tests failed ({success_count}/{len(results)} passed)")

        for result in results:
            if result["status"] == 0:
                print_and_log(f"  Attempt {result['attempt']}: ✓ Success in {result['elapsed']:.2f}s, {result['notes_count']} notes")
            elif result["status"] == -1:
                print_and_log(f"  Attempt {result['attempt']}: ✗ Error in {result['elapsed']:.2f}s - {result.get('error_message', 'Unknown error')}")
            else:
                print_and_log(f"  Attempt {result['attempt']}: ✗ Failed with status {result['status']} in {result['elapsed']:.2f}s")
...
def get_system_info() -> None:
    ...
    try:
        ...
        print_and_log(f"  OS: {platform.system()} {platform.release()} ({platform.version()})")
        ...
        if platform.system() in ('Linux', 'Darwin'):
            try:
                ...
                print_and_log("  " + output.replace('\n', '\n  ')[:500] + "..." if len(output) > 500 else output)
            except Exception as e:
...
def provide_recommendations(test_results: Dict[str, bool]) -> None:
    ...
    if not test_results.get("dns", True):
        ...
        print_and_log("  - Try using alternative DNS servers (e.g., 8.8.8.8 or 1.1.1.1)")
    ...
    if not test_results.get("tls", True):
        ...
        print_and_log("  - Check if your system date and time are correct (important for SSL certificates)")
        print_and_log("  - Update your SSL certificates")
        print_and_log("  - Check if a firewall or security software is blocking secure connections")

    if not test_results.get("http", True):
        print_and_log("🔧 HTTP connection issues detected:")
        print_and_log("  - Check if a firewall is blocking outbound connections to Simplenote")
    ...
    if not test_results.get("library", True):
        ...
        print_and_log("  - Try reinstalling the simplenote library: pip install simplenote --upgrade")
        print_and_log("  - Check the Python environment being used")

    # General recommendations
    print_and_log("\n🔧 General recommendations:")
    print_and_log("  1. Check Simplenote service status at https://status.simplenote.com/")
    print_and_log("  2. Try using the official Simplenote app to confirm account access")
    print_and_log("  3. Set LOG_LEVEL=DEBUG in your environment for more detailed logs")
    print_and_log("  4. Increase SYNC_INTERVAL_SECONDS to reduce API load (e.g., 300 for 5 minutes)")
...
async def main() -> None:
    ...
    print_and_log(f"Starting Simplenote API diagnostics at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Diagnose Simplenote API connectivity issues")
