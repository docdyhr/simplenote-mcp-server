...
async def test_list_resources():
    ...
    assert first_resource.uri.startswith("simplenote://note/"), "URI should have expected format"
    
    # Test with limit
    limited_resources = await handle_list_resources(limit=3)
    assert len(limited_resources) <= 3, f"Should return at most 3 resources, got {len(limited_resources)}"
...
async def test_read_resource():
    ...
    assert "not found" in str(exc_info.value).lower(), "Error should indicate resource not found"
    ...
    assert "invalid" in str(exc_info.value).lower(), "Error should indicate invalid URI format"
...
async def test_list_tools():
    ...
    expected_tools = {
        ...
        "search_notes", "add_tags", "remove_tags", "replace_tags"
    }
...
async def test_search_notes_tool():
    ...
    try:
        ...
        text_result = next((item for item in search_result if item.type == "text"), None)
        assert text_result is not None, "Should have text result"
        result_text = text_result.text
        assert unique_text in result_text, "Search result should contain the unique text"
...
async def test_tag_management_tools():
    ...
    try:
        ...
        assert "initial" in add_data["tags"], "Original tag 'initial' should be preserved"
        ...
        text_result = next((item for item in remove_result if item.type == "text"), None)
        ...
        text_result = next((item for item in replace_result if item.type == "text"), None)
        ...
        assert actual_tags == expected_tags, f"Tags should be exactly {expected_tags}, got {actual_tags}"
...
async def test_error_handling():
    ...
    assert "required" in str(exc_info.value).lower(), "Should indicate missing required field"
