...
from simplenote_mcp.server.errors import NetworkError, ResourceNotFoundError, ServerError
...
def add_organization_to_cache(cache_instance):
    ...
    def enhanced_get_all_notes(
        ...
        if sort_by == "modified_date" and not pinned_only and include_pinned_first is False:
...
@pytest.fixture
async def organization_test_notes(simplenote_client, organization_cache) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for note_id in note_ids:
            try:
                ...
                logger.warning(f"Error cleaning up test note {note_id}: {str(cleanup_e)}")
...
async def test_pin_unpin_note(organization_cache, test_note):
    ...
    assert cached_note.get("pinned", False) is True, "Pinned status not reflected in cache"
    ...
    assert unpinned_note.get("pinned", False) is False, "Note should be marked as unpinned"

    # Verify the note is still in the cache and marked as unpinned
    cached_note = organization_cache.get_note(note_id)
    assert cached_note.get("pinned", False) is False, "Unpinned status not reflected in cache"
...
async def test_pinned_notes_first(organization_cache, organization_test_notes):
    ...
    for sort_by in ["modified_date", "created_date", "title", "content_length"]:
        sorted_notes = organization_cache.get_all_notes(sort_by=sort_by, include_pinned_first=True)
        assert sorted_notes[0].get("key") == middle_note_id, f"Pinned note should be first with {sort_by} sorting"
...
async def test_custom_sorting_title(organization_cache, organization_test_notes):
    ...
    logger.info(f"Found {len(filtered_results)} of {len(test_note_keys)} test notes in sorted results")
    if len(filtered_results) < len(test_note_keys):
        logger.info(f"Missing keys: {set(test_note_keys) - {note['key'] for note in filtered_results}}")
...
@pytest.mark.asyncio
async def test_custom_sorting_content_length(organization_cache, organization_test_notes):
    ...
    assert content_lengths[0] > content_lengths[1], "First note should be longer than second"
    assert content_lengths[1] > content_lengths[2], "Second note should be longer than third"
    
    # The long content note (starting with B) should be first
    assert filtered_results[0]["content"].splitlines()[0].startswith("B"), "Longest note should start with B"
...
async def test_note_organization_integration(simplenote_client, organization_cache):
    ...
    try:
        ...
        # Log diagnostic information
        logger.info(f"Found {len(test_note_alpha)} of {len(note_ids)} test notes in alpha sorted results")
        ...
        pinned_sorted = organization_cache.get_all_notes(sort_by="title", include_pinned_first=True)
        ...
        # Log diagnostic information
        logger.info(f"Found {len(test_notes_pinned)} of {len(note_ids)} test notes in pinned sorted results")
        
        assert len(test_notes_pinned) == 3, "Should retrieve all test notes"
        assert test_notes_pinned[0]["key"] == z_note["key"], "Pinned Z note should be first"
