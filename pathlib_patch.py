            ...
            if patch_type == "attribute":
                ...
                for part in parent_path.split("."):
                    if hasattr(parent, part):
                        ...
                        logger.debug(
                            f"Attribute {part} not found in {parent}, skipping patch"
                        )
                        break

                # Patch the attribute
                if hasattr(parent, attr_name) and hasattr(
...
patch_dependencies()  # We need to run the patch on import for Python 3.13 compatibility
...
if __name__ == "__main__":
    ...
    if hasattr(pathlib, "Path"):
        ...
        try:
            ...
            print("   Path object creation test: ✅ Success")
        ...
        try:
            ...
            print(
                f"   anyio._core._fileio.pathlib.Path available: {hasattr(anyio._core._fileio.pathlib, 'Path')}"
            )
        except ImportError:
        ...
        print(
            "   This is a critical error that will prevent the application from running."
        )
