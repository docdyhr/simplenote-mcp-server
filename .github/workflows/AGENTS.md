...
- `evaluation-quality-gate.yml` — thin caller: `reusable-eval-quality-run.yml` (npm eval suite, gated on `ANTHROPIC_API_KEY` presence so Dependabot PRs skip gracefully; parsing via `scripts/ci/parse_eval_results.py`) → `reusable-eval-quality-report.yml` (PR comment + check-run via `.github/scripts/post-quality-gate-comment.js`/`create-quality-gate-check.js`). `mcp-evaluations.yml` — mcp-evals suite runner, gates on `OPENAI_API_KEY` presence (Dependabot/fork PRs have no repo secret access) — note this is a *different* secret than `evaluation-quality-gate.yml`'s `ANTHROPIC_API_KEY`, previously misdocumented here as the same.
